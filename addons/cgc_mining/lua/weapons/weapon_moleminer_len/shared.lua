...
function SWEP:Think()
	if (self.Owner:GetNWBool("MoleMining-Digging")) then
...
function SWEP:PrimaryAttack()
	...
	timer.Simple(0.1, function ()
		CreateTunnel(ply, ply:GetPos(), moleTrail) -- need the timer for the effect to run
	end)
	...
	timer.Simple(3, function()
		ply:SetPos(prevLoc + Vector(0, 0, 10))
		ply:Freeze(false)
		ply:SetNoDraw(true)
			...
			for i=1, 5 do
				timer.Simple(0.5 + i, function()
					if (ply:IsOnGround()) then
						local trail = ents.Create( "gmod_button" )
						if ( !IsValid( trail ) ) then return end
						trail:SetModel( "models/props/cs_militia/rocksteppingstones01.mdl" )
						trail:SetPos( ply:GetPos() )
						trail:SetCollisionGroup(11)
						trail:SetModelScale(0.4)
	...
	timer.Simple(self.DiggingTime + 4, function()
...
function CreateTunnel(ply, location, trail)
	if SERVER then
