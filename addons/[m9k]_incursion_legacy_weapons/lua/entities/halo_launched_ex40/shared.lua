...
self.Entity:SetNWBool("smoke", true)
 ...
 function ENT:Think()
			...
			return true
				...
				if tr.Hit and self.InFlight then
					if not (tr.MatType == 70 or tr.MatType == 50) then
						util.BlastDamage(self.Entity, self.Owner, tr.HitPos, 400, 90)
	...
	self.flightvector = self.flightvector - (self.flightvector/55)  + Vector(math.Rand(-0.2,0.2), math.Rand(-0.2,0.2),math.Rand(-0.1,0.1)) + Vector(0,0,-0.111)
