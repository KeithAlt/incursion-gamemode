...
SWEP.Purpose				= "Eradicate all within the area."
...
SWEP.PrintName				= "Orbital Strike"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.Sound			= Sound("weapons/radiooperator/begin.mp3")		-- script that calls the primary fire sound
...
SWEP.Primary.Ammo			= "item_ammo_ar2"
...
function SWEP:PrimaryAttack()
	...
	if (self:CanPrimaryAttack() and self.Owner:IsPlayer() and self.NextShoot <= CurTime()) and (!self.Owner:KeyDown(IN_SPEED) and !self.Owner:KeyDown(IN_RELOAD)) then
		...
		if SERVER then
			for i = 1, 20 do
		   		timer.Simple(i * 3, function()
		       		local vector = VectorRand() * 1000
		       		vector.z = 2000

		       		vector = vector + mark.HitPos

		       		local sat = ents.Create("gb_mininuke")
		       		sat:SetOwner(self:GetOwner())
		       		sat:EmitSound("ohgodno.wav", 500)
		       		sat:SetPos(vector)
		       		sat:Spawn()
		   		end)
			end
