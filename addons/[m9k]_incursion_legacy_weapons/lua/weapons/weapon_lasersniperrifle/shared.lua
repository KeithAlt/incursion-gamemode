if (CLIENT) then
	...
	SWEP.WElements = {
		...
		["lasermusket"] = { type = "Model", model = "models/weapons/laserrifle_scoped/c_laser_sniperifle_len.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-5.562, 6.105, -5.467), angle = Angle(-13.356, 0, 180), size = Vector(0.906, 0.906, 0.906), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
...
SWEP.PrintName			= "Laser Sniper Rifle"					// 'Nice' Weapon name (Shown on HUD)	
...
SWEP.ViewModel = "models/weapons/laserrifle_scoped/c_laser_sniperifle_len.mdl"
...
SWEP.Primary.Sound 			= Sound("lasersnipersounds/shoot.mp3")
...
SWEP.Secondary.UseElcan			= false -- 1

SWEP.Secondary.UseGreenDuplex	= false -- 2	
...
function SWEP:PrimaryAttack()
	...
	if ( !self:CanPrimaryAttack() ) then return end
		timer.Create( "laser_timer", 0.25, 1, function()
			self:RecoilPower()
...
function SWEP:CSShootBullet(dmg, recoil, numbul, cone)
	...
	local bullet 	= {}
	bullet.Num  	= numbul
	bullet.Src 		= self.Owner:GetShootPos() + Vector(4, 0, -2)			-- Source
	bullet.Dir 		= self.Owner:GetAimVector()			-- Dir of bullet
	bullet.Spread 	= Vector(cone, cone, 0)     						-- Aim Cone
	bullet.Tracer 	= 1       									-- Show a tracer on every x bullets
	bullet.TracerName 	= "effect_fo3_laser"
	bullet.Force 	= 0.5     								-- Amount of force to give to phys objects
...
-- 	bullet.Callback	= function ( a, b, c ) BulletPenetration( 0, a, b, c ) end 	-- CALL THE FUNCTION BULLETPENETRATION
	...
	local righthand = self:LookupAttachment("anim_attachment_RH")
	local posang = self:GetAttachment(righthand)
...
function SWEP:Reload()
	...
	if ( self.Weapon:Clip1() < self.Primary.ClipSize ) and self.Owner:GetAmmoCount(self.Primary.Ammo) > 0 then
		...
		self:EmitSound( "lasersnipersounds/reload.mp3" )
