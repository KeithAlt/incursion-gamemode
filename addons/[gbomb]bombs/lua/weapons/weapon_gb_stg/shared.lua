...
SWEP.ViewModel = "models/weapons/gbombs/c_stgm24.mdl"
	...
	if (CLIENT) then 	
		function func_Aim()
			...
			local function func_DrawAimText()
				...
				draw.DrawText( "Ammo remaining: "..LocalPlayer():GetNWInt("ammo_stg"), "Rescaled", ScrW() / 1.99, ScrH() / (1.8 - (velocity / 300)) , Color(255,255,255,255), TEXT_ALIGN_CENTER )
			end
...
function SWEP:TakeAmmo()
	...
	if self.Owner:GetNWInt("ammo_stg") > 0 then
		self.Owner:SetNWInt("ammo_stg", self.Owner:GetNWInt("ammo_stg") - 1 )
		return true
	else
		return false
	end
