...
SWEP.ViewModel = "models/weapons/gbombs/c_swhite.mdl"
	...
	if (CLIENT) then 	
		function func_Aim()
			...
			local function func_DrawAimText()
				...
				draw.DrawText( "Ammo remaining: "..LocalPlayer():GetNWInt("ammo_swhite"), "Rescaled", ScrW() / 1.99, ScrH() / (1.8 - (velocity / 300)) , Color(255,255,255,255), TEXT_ALIGN_CENTER )
			end
...
function SWEP:TakeAmmo()
	...
	if self.Owner:GetNWInt("ammo_swhite") > 0 then
		self.Owner:SetNWInt("ammo_swhite", self.Owner:GetNWInt("ammo_swhite") - 1 )
		return true
	else
		return false
	end
...
function SWEP:PrimaryAttack()
	...
	local ent = ents.Create("gb_swhite")
