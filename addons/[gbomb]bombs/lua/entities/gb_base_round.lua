...
function ENT:Initialize()
 if (SERVER) then
	 ...
	 self.Arming   = false
	 self.CanLaunch = true
	 if !(WireAddon == nil) then self.Inputs = Wire_CreateInputs(self, { "Arm", "Detonate", "Fire" }) end
	end
end
...
function ENT:OnTakeDamage(dmginfo)
	 ...
	 if(self.Fired and !self.Burnt and self.Armed) then
	     if (dmginfo:GetDamage() >= 2) then
	         ...
	         phys:AddAngleVelocity(dmginfo:GetDamageForce()*5)
	     end
	 end
...
function ENT:PhysicsCollide( data, physobj )
	 ...
	 if(self.Life <= 0) then return end
	 if(GetConVar("gb_fragility"):GetInt() >= 1) then
	     if(!self.Fired and !self.Burnt and !self.Arming and !self.Armed) and (data.Speed > self.ImpactSpeed) then
		     if(math.random(0,9) == 1) then
			     self:Launch()
		         self:EmitSound(damagesound)
			 else
...
function ENT:Launch()
	 ...
	 timer.Simple(self.IgnitionDelay,function()
		 ...
		 ParticleEffectAttach(self.Trail,PATTACH_ABSORIGIN_FOLLOW,self,1)
     end)		 
end
