...
function ENT:Explode()
	 ...
	 for i=0, (self.NumBomblets-1) do
		 ...
		 if self.GBOWNER == nil then
			bomblet:SetVar("GBOWNER",table.Random(player.GetAll()))
		 else
...
function ENT:Think()
	 ...
	 local tipforce = self:GetVelocity() * self:GetUp() / (phys:GetMass() * phys:GetMass() * 10)
	 
	 --phys:AddAngleVelocity(tipforce) --Cheat->we want to tip our nose down
