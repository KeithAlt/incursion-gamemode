...
function ENT:Use( activator, caller )
	...
	if !activator:KeyDown( IN_RELOAD ) then
		...
		activator:SetNWInt("ammo_spurple", activator:GetNWInt("ammo_spurple") + 1)

		if !activator:HasWeapon( "weapon_gb_sviolet" ) then
			activator:Give("weapon_gb_sviolet")
		end
		self:Remove()
	end
end
