...
function ENT:Use( activator, caller )
	...
	if !activator:KeyDown( IN_RELOAD ) then
		...
		activator:SetNWInt("ammo_sred", activator:GetNWInt("ammo_sgred") + 1)

		if !activator:HasWeapon( "weapon_gb_sred" ) then
			activator:Give("weapon_gb_sred")
		end
		self:Remove()
	end
end
