...
function CardGameCl:getCardStartPosition()
	local e = self:getTableEntity()
	if IsValid(e) then
		return e:SeatToLocal(0, 25)
	end
...
function CardGameCl:createCardObject(card, endPos)
	local obj = {
		anim = {
			...
			startPos = self:getCardStartPosition(),
			endPos = endPos
		},
		card = card
	}
...
function CardGameCl:updateCardEndPos(ent, card, idx, ply, seatIndex)
	local _, lang = ent:SeatToLocal(seatIndex)
	local lpos, _ = ent:SeatToLocal(seatIndex + self:getCardHorizOffset(idx), self:getCardVertOffset(idx))
	card.anim.endPos = lpos
	card.anim.endPos.z = 0
	card.anim.endYaw = lang.y
end

-- Returns the amount of chips that should be shown on the chip stack in front of player
function CardGameCl:getGModPlayerChipCount(ply)
...
function CardGameCl:drawTable(ent, localOrigin)
	for _,tpl in pairs(ent:GetCachedSeatindexPlyTuples()) do
		...
		if ply:IsValid() then
			...
			local chips = self:getGModPlayerChipCount(ply)
			if chips > 0 then
				CasinoKit.drawChipStack(chips, lpos, lang, true)
			end
		end
	end
