...
function GameCl:handleGameMessage(id, buf)
	...
	elseif id == "glmsg" then
		local id = buf:getString()

		local params = {}
		for i=1,buf:get() do
			local k, v = buf:getString(), buf:getString()
			params[k] = v
		end

		self:onGameplayMessageReceived(CasinoKit.L(id, params))
	else
...
-- Gets local player's seat index
function GameCl:getMySeatIndex()
	local te = self:getTableEntity()
	return IsValid(te) and te:GetMySeatIndex()
end

-- Is local player sitting on table that this game is happening on
function GameCl:isLocalPlayerSitting()
	local te = self:getTableEntity()
	return IsValid(te) and te:IsLocalPlayerSitting()
end
...
function GameCl:getHelpHTML()
	return self.htmlHelp
end
