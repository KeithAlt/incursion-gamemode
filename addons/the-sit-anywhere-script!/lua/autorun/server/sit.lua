...
local function Sit(ply, pos, ang, parent, parentbone,  func, exit)
	...
	if (ply:GetNWBool("StealthCamo", true)) then -- Prevents a Stealth Boy SWEP exploit that allows for infinite Invisibility
...
concommand.Add("unsit",function(ply, cmd, args)
	if SERVER then
		ply:ExitVehicle()
	end
end)
