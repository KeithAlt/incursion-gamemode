...
function ENT:Initialize()  
	...
	self.flightvector = self.Entity:GetForward() *  100--((80*76.5)/100) --6
			...
			return true
				...
				if tr.Hit and self.InFlight then
					if not (tr.MatType == 70 or tr.MatType == 50) then
						util.BlastDamage(self.Entity, self.Owner, tr.HitPos, 200, 125)
					...
					self.Entity:Remove()
						...
						if (tr.Entity:IsPlayer() or tr.Entity:IsNPC()) then tr.Entity:TakeDamage(125, self.Owner, self.Entity)	end
						local effectdata = EffectData()
						effectdata:SetOrigin(tr.HitPos)			// Where is hits
						effectdata:SetNormal(tr.HitNormal)		// Direction of particles
						effectdata:SetEntity(self.Entity)		// Who done it?
