...
ENT.DamageDirect = 200 -- Damage on impact, adds on with the blast radius damage but as true damage without radius involved.
ENT.DamageRadius = 300 -- Radius of explosion
...
if SERVER then
    ...
    function ENT:Explosion( ent )
		if IsValid(ent) and ent:IsPlayer() then
			ent:TakeDamage(self.damage * 10, self:GetOwner(), self:GetOwner()) -- Apply extra damage due to being direct hit.
