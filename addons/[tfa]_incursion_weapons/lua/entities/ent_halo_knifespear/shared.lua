...
function ENT:Use(activator, caller) 
	...
	if ((gmod.GetGamemode().Name) == "Murderthon 9000") or (GetConVar("DebugM9K"):GetBool()) then
		if (activator:IsPlayer()) then
			if activator:GetWeapon("weapon_knifespear") == NULL 
			&& activator:GetWeapon("m9k_machete") == NULL then
				activator:Give("weapon_knifespear")
				self.Entity:Remove()
			else return 
			end
		end
	elseif (activator:IsPlayer()) then
		if activator:GetWeapon("weapon_knifespear") == NULL then
			activator:Give("weapon_knifespear")
			self.Entity:Remove()
		else
			activator:GiveAmmo(1, "Knife_Spear")
			self.Entity:Remove()
		end
	end
end
