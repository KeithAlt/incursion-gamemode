...
SWEP.Category               = "Fallout SWEPs - Laser Weapons" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.PrintName              = "Laser Musket (Scoped)"       -- Weapon name (Shown on HUD)
...
SWEP.Primary.Limit = 200 -- Number it should not pass.
SWEP.Primary.DamageBase = 70 -- Fallback number, change this to what it will reset to when shot


--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("weapons/lasermusket/LaserMusketFire.wav") -- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.HullSize = 3 --Big bullets, increase this value.  They increase the hull size of the hitscan bullet.
...
SWEP.Primary.ReloadSound = Sound("weapons/lasermusket/LaserMusketCranker.wav")
...
--[[VIEWMODEL]]--
SWEP.ViewModel          	= "models/weapons/c_lasermusket_test_5.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel         = "models/Halokiller38/fallout/weapons/Energy Weapons/laserrifle.mdl" -- Weapon world model path
...
SWEP.RunSightsAng = Vector(-7.996, 21.997, 0) --Change this, using SWEP Creation Kit preferably
...
SWEP.IronSightsPos = Vector(-4.72, -4.222, 1.399)
...
--Tracer Stuff
SWEP.TracerName         = "pcf_keith_tracer_laser"   --Change to a string of your tracer name.  Can be custom. There is a nice example at https://github.com/garrynewman/garrysmod/blob/master/garrysmod/gamemodes/base/entities/effects/tooltracer.lua
SWEP.TracerCount        = 1     --0 disables, otherwise, 1 in X chance
--Impact Effects
SWEP.ImpactEffect = nil--Impact Effect
SWEP.ImpactDecal = nil--Impact Decal
SWEP.CrankDamage = 0 -- DO NOT CHANGE THIS !!!!!!!!! SEE THE SWEP:PrimaryAttack() function in the base to change.
--[[EVENT TABLE]]--
...
--[[ATTACHMENTS]]--
SWEP.VElements = {
	--["scatter"] = { type = "Model", model = "models/weapons/tribeam/diffuser.mdl", bone = "LaserMusketFrame", rel = "", pos = Vector(4.531, -19.129, -4.173), angle = Angle(-0.969, 80.839, -89.131), size = Vector(0.632, 0.632, 0.632), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = false },
	--["hr_scope_rt"] = { type = "Model", model = "models/rtcircle.mdl", bone = "LaserMusketFrame", rel = "", pos = Vector(10.114, -11.12, 0), angle = Angle(0, -98.929, 180), size = Vector(0.218, 0.218, 0.218), color = Color(255, 255, 255, 255), surpresslightning = false, material = "!tfa_rtmaterial", skin = 0, bodygroup = {}, bonemerge = false, active = false },
	["hr_scope"] = { type = "Model", model = "models/weapons/fallout4guns/lasermusket_scope_test.mdl", bone = "LaserMusketFrame", rel = "", pos = Vector(8.991, -14.693, 0), angle = Angle(0.735, 80.932, 89.751), size = Vector(0.568, 0.568, 0.568), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = true }
}

SWEP.LMScopePos = Vector(-6, -10, 0.15)
SWEP.LMScopeAng = Vector(0, 0, 0)

--Export from SWEP Creation Kit.  For each item that can/will be toggled, set active=false in its individual table
SWEP.WElements = {
    ["lasermusket"] = { type = "Model", model = "models/weapons/c_lasermusket_test_5.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-5.562, 6.105, -5.467), angle = Angle(-13.356, 0, 180), size = Vector(0.906, 0.906, 0.906), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["lasermusket_scope"] = { type = "Model", model = "models/weapons/fallout4guns/lasermusket_scope_test.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(9.299, 0.5, -7), angle = Angle(-11, 0, 176), size = Vector(.5, .5, .5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = true },
}
...
function SWEP:Reload(released)
	...
	if self:Clip1() >= 7 then return end
...
function SWEP:PrimaryAttack()
	...
	elseif self:Clip1() == 2 then
		...
		self.Primary.Damage = self.Primary.Damage + 10

	elseif self:Clip1() == 3 then

		self.Primary.Damage = self.Primary.Damage + 30

	elseif self:Clip1() == 4 then

		self.Primary.Damage = self.Primary.Damage + 45

	elseif self:Clip1() == 5 then

		self.Primary.Damage = self.Primary.Damage + 50

	elseif self:Clip1() == 6 then

		self.Primary.Damage = self.Primary.Damage + 65

	elseif self:Clip1() == 7 then

		self.Primary.Damage = self.Primary.Damage + 80

	end

	self:SetClip1(0)
	self:ShootBulletInformation()
	self.Primary.Damage = self.Primary.DamageBase -- reset damage
