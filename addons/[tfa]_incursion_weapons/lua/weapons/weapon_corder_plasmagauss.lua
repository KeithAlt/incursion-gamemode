...
SWEP.Category           	= "Fallout SWEPs - Custom Orders" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.PrintName              = "Plasma Gauss"       -- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("cgc/plasmagauss/fire.mp3") -- This is the sound of the weapon, when you shoot.
...
SWEP.PercentageDamage = .10 -- Percentage of hp which is added on to damage
SWEP.Plasma = true

// max health - % health, max % health - % damage
hook.Add("EntityTakeDamage", "TFA_PercentHPCalculation", function(ent, dmg)
	if !ent:IsPlayer() then return end

	local attacker = dmg:GetAttacker()
	if !attacker:IsPlayer() then return end
	local wep = attacker:GetActiveWeapon()
	if IsValid(wep) and wep.PercentageDamage and (ent:GetMaxHealth() > 151 or (Armor and ent:WearingPA())) then
		local hdmg = wep.PercentageDamage
		local addedDamage = ent:GetMaxHealth() * hdmg
		local newDmg = addedDamage + dmg:GetDamage()

		dmg:SetDamage(newDmg)

		if SERVER and wep.Plasma then
			ParticleEffectAttach( "mr_acid_trail", 4, ent, ent:LookupAttachment( "chest" ))
...
SWEP.JamFactor = .5 -- How to increase jam factor after each shot.
...
SWEP.FireModeName = "Plasma Bolt" --Change to a text value to override it
...
SWEP.Primary.Ammo = "MicrofusionCell" //"MicrofusionCell" -- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.
...
--[[VIEWMODEL]]--
SWEP.ViewModel			    = "models/weapons/cgc/c_plasmagauss_len.mdl" --Viewmodel path
...
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			=  "models/weapons/customorders3/w_m2browning_len.mdl" -- Weapon world model path
...
--[[SPRINTING]]--
SWEP.RunSightsPos = Vector(5.27, -5.473, -5.736)
SWEP.RunSightsAng = Vector(0, 49.967, 0)
--[[IRONSIGHTS]]--
--[[IRONSIGHTS]]--
SWEP.data = {}
...
SWEP.IronSightsPos = Vector(-3.9, 0, .2)
SWEP.IronSightsAng = Vector(0, 0, 0)
--[[INSPECTION]]--
SWEP.InspectPos = Vector(6, -8, -1)--Vector(0,0,0) --Replace with a vector, in style of ironsights position, to be used for inspection
...
--Tracer Stuff
SWEP.TracerName         = "pcf_keith_tracer1_acid1"   --Change to a string of your tracer name.  Can be custom. There is a nice example at https://github.com/garrynewman/garrysmod/blob/master/garrysmod/gamemodes/base/entities/effects/tooltracer.lua
...
SWEP.WElements = {
	["plasmagauss"] = { type = "Model", model = "models/weapons/cgc/c_plasmagauss_len.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-4.232, 5.526, -7.638), angle = Angle(-8.907, 0, 180), size = Vector(0.994, 0.994, 0.994), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

sound.Add( {
	name = "PlasmaGauss.Crank",
	channel = CHAN_STATIC,
	volume = 1.0,
	level = 80,
	pitch = {95, 110},
	sound = "cgc/plasmagauss/crank.mp3"
} )

sound.Add( {
	name = "PlasmaGauss.CrankDown",
	channel = CHAN_STATIC,
	volume = 1.0,
	level = 80,
	pitch = {95, 110},
	sound = "cgc/plasmagauss/crankdown.mp3"
} )

sound.Add( {
	name = "PlasmaGauss.CellTake",
	channel = CHAN_STATIC,
	volume = 1.0,
	level = 80,
	pitch = {95, 110},
	sound = "cgc/plasmagauss/cell.mp3"
} )
