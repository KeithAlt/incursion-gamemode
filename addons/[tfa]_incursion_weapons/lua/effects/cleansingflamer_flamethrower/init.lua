...
if data:GetEntity():IsValid() && StartPos && HitPos then
	...
	local FlameEmitter = ParticleEmitter(StartPos)
		...
		for i=0, 8 do
			...
			local FlameMat = {}
			FlameMat[1] = "effects/cleansingflamer/blueflame"
			FlameMat[2] = "effects/cleansingflamer/blueflame"
			FlameMat[3] = "effects/cleansingflamer/blueflame"
			...
			if (FlameParticle) then
				...
				FlameParticle:SetVelocity(((HitPos - StartPos):GetNormal() * math.random(1720,2820)) + (VectorRand() * math.random(142,172)) )
				...
				FlameParticle:SetEndSize(math.random(32,52)) -- 52
	...
	local SmokeEmitter = ParticleEmitter(StartPos)
		...
		for i=0, 2 do
			...
			SmokeParticle = SmokeEmitter:Add( "effects/flamesmoke.vtf" .. math.random(1,8) .. "", StartPos )
