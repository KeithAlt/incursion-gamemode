...
SWEP.WepName="weapon_cattleprod_m2"
...
function SWEP:AttackAnimation()
	...
	self.Weapon:EmitSound("impacts/laser_impact_3.wav")
end
...
local randomSounds = {
	"impacts/laser_impact_1.wav",
	"impacts/laser_impact_2.wav",
	"impacts/laser_impact_1.wav",
	"impacts/laser_impact_1.wav",
}
function SWEP:AtkExtra()
	...
	if ( IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer()) ) then
		...
		if SERVER then
			local randomDir = AngleRand(-90, 90)
			tr.Entity:ViewPunch(randomDir)
			tr.Entity:Freeze(true)
			tr.Entity:EmitSound("impacts/laser_impact_" .. math.random(1,4) .. ".wav")
			tr.Entity:SetVelocity(ply:GetForward() * 500)

			timer.Simple(0, function()
				ParticleEffectAttach("mr_electric_1", PATTACH_POINT_FOLLOW, tr.Entity, 1)
			end)
...
SWEP.VElements = {
	["plasmasword"] = { type = "Model", model = "models/halokiller38/fallout/weapons/melee/cattleprod.mdl", bone = "RW_Weapon", rel = "", pos = Vector(1, 0, -10.903), angle = Angle(0, 180, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["plasmasword"] = { type = "Model", model = "models/halokiller38/fallout/weapons/melee/cattleprod.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(4.104, 1.5, 10.139), angle = Angle(0, 90.751, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
