...
local EnemyDistance = self:VJ_GetNearestPointToEntityDistance(self:GetEnemy(),self:GetPos():Distance(self:GetEnemy():GetPos()))
	if EnemyDistance > 0 && EnemyDistance < 25 then
		...
		self.MeleeAttackAngleRadius = 150 -- What is the attack angle radius? | 100 = In front of the SNPC | 180 = All around the SNPC
		self.MeleeAttackDamageAngleRadius = 150 -- What is the damage angle radius? | 100 = In front of the SNPC | 180 = All around the SNPC
