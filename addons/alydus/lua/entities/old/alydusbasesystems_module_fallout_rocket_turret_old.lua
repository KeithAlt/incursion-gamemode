...
--this is an old file that i kept for reference
	...
	if SERVER then
		...
		for potentialTargetKey, potentialTarget in pairs(ents.FindInSphere(self:GetPos() + Vector(0, 0, 30), GetConVar("sv_alydusbasesystems_config_samturret_range"):GetInt())) do
			if IsValid(potentialTarget) then
				if potentialTarget:IsVehicle() then
					canFire = true
					if canFire then
						table.insert(foundPotentialTargetsVehicles, potentialTarget)
					end
				elseif potentialTarget:GetClass() == "npc_helicopter" or potentialTarget:GetClass() == "npc_combinegunship" then
					table.insert(foundPotentialTargetsVehicles, potentialTarget)
...
ENT.PrintName = "Module - FO4 Rocket Turret"
ENT.Author = "Alydus"
ENT.Contact = "Alydus"
ENT.Purpose = ""
ENT.Instructions = ""