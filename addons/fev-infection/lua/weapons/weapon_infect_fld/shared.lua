...
SWEP.WerewolfModel	= "models/player/fallout_3/glowing_one.mdl"
...
function SWEP:Deploy()
	if CLIENT then
		chat.AddText(
		Color( 0, 200, 0 ), "FEV Infection:")
		chat.AddText(
		Color( 255, 100, 0 ), "You are infected.",
		Color( 255, 255, 0 ), "Do not reveal yourself.",
		Color( 255, 100, 0 ), "You must spread the infection further.",
		Color( 255, 255, 0 ), "You have the ability to transform between infected and human.",
		Color( 255, 100, 0 ), "Take out the Infection and right-click to transform.",
		Color( 255, 255, 0 ), "Toggle hunter vision by pressing 'E'.")
	end
...
function SWEP:MorphToHuman()
	local owner = self.Owner
	if owner == nil or !IsValid(self) or !self.PlayerModel then return end
	if owner:GetModel() == self.PlayerModel then return end
...
function SWEP:Infection(dmg)
	...
	if SERVER then dmg:ChatPrint("You have been infected by a FEV Ghoul, you will turn in "..(self.InfectDuration*5).." seconds.") end
