...
self.tMats = {}
self.tMats.Glow1 = Material("sprites/light_glow02")
self.tMats.Glow2 = Material("sprites/flare1")

for _,mat in pairs(self.tMats) do
	...
	self.FlashSize = 1
		...
		for i=1,math.ceil(self.Scale*250) do
			...
			particle:SetDieTime(math.Rand(10.5,18.9)*self.Scale)
			...
			particle:SetGravity(Vector(math.Rand(-200,200),math.Rand(-200,200),0))
			particle:SetAirResistance(100)
			particle:SetDieTime(math.Rand(5.5,8.9)*self.Scale)
		...
		for i=1,math.ceil(self.Scale*30) do
			...
			particle:SetAirResistance(100)
			particle:SetDieTime(math.Rand(15.5,20.9)*self.ScaleSlow)
			particle:SetStartAlpha(100)
			particle:SetEndAlpha(0)
			particle:SetStartSize(size)
			particle:SetEndSize(size)
...
--smoke mushroom
		for i=1,math.ceil(self.Scale*25) do
				...
				local particle = emitter:Add("particles/Dirt",   spawnpos - vecang*9*i)
		...
		--mushroom embers
				...
				local particle = emitter:Add("effects/fire_embers"..math.random(1,3),   spawnpos - vecang*9*i)
                        local dietime = math.Rand(10.5,10.9)*self.Scale
				particle:SetVelocity(velocity*self.Scale)
				particle:SetDieTime(dietime)
		...
		for i=1,math.ceil(self.Scale*10) do
				...
				local particle = emitter:Add("particles/FireExplosion",   spawnpos - vecang*9*i)
                        local dietime = math.Rand(10.5,10.9)*self.Scale
				particle:SetVelocity(velocity*self.Scale)
				particle:SetDieTime(dietime)
	...
	emitter:Finish()
		...
		surface.PlaySound("wep/explode/fx_explosion_nuke_small_2d.mp3")
		self.Entity:EmitSound("wep/explode/fx_explosion_nuke_small_3d.mp3",511,110,1)
...
function EFFECT:Think()
	...
	if TimeLeft > 0 then 
		...
		self.FlashAlpha = self.FlashAlpha - 100*FTime
...
if !self.tMats or self.Scale < 1 then return end

--base glow
render.SetMaterial(self.tMats.Glow1)
...
--blinding flash
	if self.FlashAlpha > 0 then
		render.SetMaterial(self.tMats.Glow2)
