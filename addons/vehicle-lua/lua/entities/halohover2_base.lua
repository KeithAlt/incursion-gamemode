...
ENT.PrintName = "Halo Hover 2 Base"
ENT.Author = "Liam0102"
ENT.Category = "Halo"
ENT.AutomaticFrameAdvance = true
ENT.Spawnable = false;
ENT.AdminSpawnable = false;
ENT.IsHALOWALKERVehicle = true;
...
function IsHALOWALKERSpeeder(class)
...
if SERVER then
function HALOWALKERCreateBulletStructure(dmg,color,nosplashdamage)
	...
	local bullet = {
		...
		Callback = function(p,tr,damage)
			...
			util.Effect( "ImpactGunship", fx, true )
			...
			if(IsValid(self) and self != tr.Entity) then
				...
				if(tr.Entity.IsHALOWALKERVehicle and tr.Entity.Inflight) then
				...
				if(color == "blue" and !noion) then
					if(tr.Entity.IsHALOWALKERVehicle) then
...
function ENT:Enter(p,driver)
	...
	p:SetNWInt("HALOWALKER_Speeder_MaxSpeed",self.BoostSpeed)
...
hook.Add("PlayerEnteredVehicle","HALOWALKERSpeederUse", function(p, e)
	if(IsValid(e) and IsValid(p) and p:IsPlayer()) then
		local speeder = e:GetParent();
		if(IsValid(speeder) and speeder.IsHALOWALKERVehicle) then
		...
		if(self.Inflight and IsValid(self.Pilot)) then
			...
			*/
			...
			if(self.EngineOn) then
				...
				if(IsValid(self.Pilot)) then
					self.Pilot:SetNWInt("HALOWALKER_Speeder_Speed",self.Accel.FWD);
				end

				if(self.Pilot:KeyDown(IN_MOVERIGHT)) then
					self.YawAccel = -10 - (self.Accel.FWD/100);
...
function HALOWALKERSpeederStandbyPickup(p,e)
	if(IsValid(e) and e.IsHALOWALKERVehicle) then
		e.ShouldStandby = false;
	end
end
hook.Add( "PhysgunPickup", "HALOWALKERSpeederStandbyPickup", HALOWALKERSpeederStandbyPickup )

function HALOWALKERSpeederStandbyDrop(p,e)
	if(IsValid(e) and e.IsHALOWALKERVehicle) then
		e.ShouldStandby = true;
	end
end
hook.Add("PhysgunDrop", "HALOWALKERSpeederStandbyDrop", HALOWALKERSpeederStandbyDrop);
...
function ENT:Bang(p) --######## The effect, and killing the player if they're flying @RononDex
	...
	self:EmitSound(Sound("haloexplosion.wav"),100,100); --Play the jumper's explosion sound(Only good explosion sound i have)
	local fx = EffectData();
		fx:SetOrigin(self:GetPos());
	util.Effect("HALOExplosion",fx);
...
if CLIENT then
	...
	function ENT:Think()
		...
		if self:GetNWInt("Speed") >= 5 then
			...
			Speed = p:GetNWInt("HALOWALKER_Speeder_Speed") or self:GetNWInt("Speed");
			MaxSpeed = p:GetNWInt("HALOWALKER_Speeder_MaxSpeed") or self:GetNWInt("MaxSpeed");
	...
	function HALOWALKER_Speeder_DrawHull(StartHealth)

		if(LocalPlayer().HALOWALKER_HideHud) then return end;
	...
	function HALOWALKER_Speeder_DrawSpeedometer()
	...
	function HALOWALKER_CovenantHoverReticles(self,WeaponsPos)
		local p = LocalPlayer();
		...
		for i=1,table.Count(WeaponsPos) do
			...
			if(IsValid(tr.Entity)) then
				if(IsHALOWALKERSpeeder(tr.Entity:GetClass()) and tr.Entity != self) then
	...
	function HALOWALKER_Cannon_Reticles(self,WeaponsPos)
		local p = LocalPlayer();
