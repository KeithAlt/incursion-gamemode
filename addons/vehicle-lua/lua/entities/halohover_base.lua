	...
	local bullet = {
		...
		Callback = function(p,tr,damage)
			...
			if(IsValid(self) and self != tr.Entity) then
				...
				if(tr.Entity.IsHALOVehicle and tr.Entity.Inflight) then
...
function ENT:Enter(p,driver)
	...
	p:SetNWInt("HALO_Speeder_MaxSpeed",self.BoostSpeed)
...
hook.Add("PlayerEnteredVehicle","HALOSpeederUse", function(p, e)
	if(IsValid(e) and IsValid(p) and p:IsPlayer()) then
		local speeder = e:GetParent();
		if(IsValid(speeder) and speeder.IsHALOVehicle) then
		...
		if(self.Inflight and IsValid(self.Pilot)) then
			...
			*/
			...
			if(self.EngineOn) then
				...
				if(IsValid(self.Pilot)) then
					self.Pilot:SetNWInt("HALO_Speeder_Speed",self.Accel.FWD);
				end
...
function HALOSpeederStandbyPickup(p,e)
	if(IsValid(e) and e.IsHALOVehicle) then
		e.ShouldStandby = false;
	end
end
hook.Add( "PhysgunPickup", "HALOSpeederStandbyPickup", HALOSpeederStandbyPickup )

function HALOSpeederStandbyDrop(p,e)
	if(IsValid(e) and e.IsHALOVehicle) then
		e.ShouldStandby = true;
	end
end
hook.Add("PhysgunDrop", "HALOSpeederStandbyDrop", HALOSpeederStandbyDrop);
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(SpeederActive) then
			...
			Speed = p:GetNWInt("HALO_Speeder_Speed") or self:GetNWInt("Speed");
			MaxSpeed = p:GetNWInt("HALO_Speeder_MaxSpeed") or self:GetNWInt("MaxSpeed");
	...
	function HALO_Speeder_DrawHull(StartHealth)
	...
	function HALO_Speeder_DrawSpeedometer()
	...
	function HALO_CovenantHoverReticles(self,WeaponsPos)
		local p = LocalPlayer();
		...
		for i=1,table.Count(WeaponsPos) do
			...
			if(IsValid(tr.Entity)) then
				if(IsHALOSpeeder(tr.Entity:GetClass()) and tr.Entity != self) then
	...
	function HALO_Cannon_Reticles(self,WeaponsPos)
		local p = LocalPlayer();
