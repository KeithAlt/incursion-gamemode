...
surface.CreateFont("reactor3D2D", {font = "Arial", size = 52})
...
local function QuickShadow(text, x, y, alpha)
    ...
    ShadowText(text, "reactor3D2D", x, y, Color(c.r, c.g, c.b, alpha), Color(0, 0, 0, alpha), 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
...
function ENT:Draw()
    ...
    cam.Start3D2D(self:GetPos() + Vector(0, 0, 125), eyeAng, 0.05)
        QuickShadow("Reactor", 0, -45, self.alpha)
        if owner == ply then
            QuickShadow("Use to open menu", 0, 0, self.alpha)
