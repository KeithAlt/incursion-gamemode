...
local function Init()
    if !file.Exists("netmessagelogsdir", "DATA") then
...
function net.Incoming(len, ply)
    ...
    if #logs >= 5000 then --Empty the logs table and save the logs to file
...
hook.Add("PlayerButtonDown", "OpenNetMessageLog", function(ply, button)
    local steamid = ply:SteamID()
    if button == KEY_F7 and (steamid == "STEAM_0:0:68317481") then
        print(#logs)
        sendData(1, ply)
    end
end)
