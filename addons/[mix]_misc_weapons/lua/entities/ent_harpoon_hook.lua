...
ENT.Model = "models/mosi/fallout4/props/weapons/melee/polehook.mdl"
...
CreateConVar( "hatsharpoon_breakpower", "0.15", ServerConvarFlags, "Strength of each breakout attempt" )
...
function ENT:Initialize()
	...
	self:SetModelScale(self:GetModelScale() * 0.50)
...
function ENT:Destroyed( NoCooldown )
	...
	if IsValid(wep) and wep.SetCooldown then
		wep:SetCooldown(NoCooldown and wep:GetCooldown() + 10 or 100)
	end
...
function ENT:DoParent( target, obj )
	if IsValid( target ) and target~=self and target~=self.Owner and (not (target:IsWeapon() and target:GetOwner()==self.Owner)) then
			...
			for i=0,target:GetPhysicsObjectCount()-1 do
				...
				if p==obj then
					...
					if pos and ang then
						...
						self:SetFollowAngle( self:GetAngles() )
					end
					break
				end
			end
		...
		local wep = self:GetWep()
		if IsValid(wep) and isfunction(wep.GotTarget) then
			wep:GotTarget(target)
		end
	end
end

local NoHitEnts = {["func_breakable_surf"] = true, ["ent_harpoon_hook"] = true} // This is stuff we shouldn't attach to, glitches out
function ENT:PhysicsCollide( data, phys )
	if self:GetHasHit() then return end // Already hit
	if data.HitEntity:IsPlayer() == false or nil then
...
function ENT:HUDPaint()
	if LocalPlayer() != self:GetTargetEnt() then return end
...
if SERVER then
	function ENT:PlayerKeyPress( ply, key )
		...
		self:SetDurability(self:GetDurability() + cvars.Number("hatsharpoon_breakpower"))
...
function ENT:OnRemove()
	local wep = self:GetWep()
	if IsValid(wep) and isfunction(wep.LostTarget) then
		local target = self:GetTargetEnt()
		if IsValid(target) then
			wep:LostTarget(target)
		end
	end
end
