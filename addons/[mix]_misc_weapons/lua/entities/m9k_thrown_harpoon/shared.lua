	...
	elseif Ent.Health then
		...
		if (Ent:IsPlayer() or Ent:IsNPC() or Ent:GetClass() == "prop_ragdoll") then
			...
			if Ent:IsPlayer() then
				...
				if chance >= 0.3 && IsValid(Ent) then
					local RunSpeed = Ent:GetRunSpeed()
				Ent:SetRunSpeed(75)
				Ent:ChatPrint("( Hit by Spear // Speed Debuff // 8 Seconds )")
				self.Owner:ChatPrint(">> Target Speed Reduced <<")


					timer.Simple(8, function()
						if !IsValid(Ent) then return end 
						Ent:SetRunSpeed(RunSpeed)
						Ent:ChatPrint("( Speed Debuff Expired )")
					end)
				end

				if chance < 0.3 && IsValid(Ent) then
					Ent:ConCommand("say /fallover 5")
					Ent:ChatPrint("( Hit by Spear // Critical Hit // 5 Second Knockout )")
					self.Owner:ChatPrint(">> Target Unconcious <<")
				end
