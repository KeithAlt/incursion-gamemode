...
if (CLIENT) then
	...
	SWEP.Purpose = "A grenade which freezes any player standing around its blast radius."
	SWEP.WepSelectIcon = surface.GetTextureID("weapons/weapon_sh_cryogrenade")
...
game.AddAmmoType({name = "Cryo Grenades"})

SWEP.Primary.ClipSize = 1
SWEP.Primary.DefaultClip = 2
SWEP.Primary.Ammo = "Cryo Grenades"
...
SWEP.ThrownProjectile = "proj_sh_cryogrenade"
SWEP.ThrowDelay = 1

-- SWEP defaults, overriden by cryogrenade_settings.lua.
SWEP.AllowCooking = true
SWEP.CookTime = 3
SWEP.ThrowForce = 1250

function SWEP:SetupVars()
	self.AllowCooking = CRYOGRENADE_SETTINGS.AllowCooking
	self.CookTime = CRYOGRENADE_SETTINGS.CookTime
	self.ThrowForce = CRYOGRENADE_SETTINGS.ThrowForce
	self.detonate_timer = self.CookTime

	if (!CRYOGRENADE_SETTINGS.AllowAmmo) then
...
if (SERVER) then
	...
	function SWEP:PreDrawViewModel(vm)
		if (vm) then
			vm:SetSubMaterial(0, "models/weapons/v_models/eq_cryogrenade/cryogrenade")
		end
	end
