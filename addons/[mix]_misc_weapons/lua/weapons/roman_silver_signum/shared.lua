...
SWEP.ViewModel = ""
SWEP.BuffExpireTime = 30 // In seconds
...
function SWEP:PrimaryAttack()
	...
	self:EmitSound("warcry/drum" .. math.random(1,5) .. ".ogg", 75, 110, 1)
...
if SERVER then
		...
		self.SecondaryCooldown = CurTime() + 50

		local ply = self.Owner
		ply:EmitSound("warcry/horn.ogg")

		for index, nearPly in pairs(ents.FindInSphere(ply:GetPos(), 800)) do
			if nearPly:IsPlayer() and nearPly:Alive() and !nearPly:GetNoDraw() and !nearPly.SpeedBannerBuffActive and nearPly:Team() == ply:Team() then
				nearPly:AddSpeed(10, self.BuffExpireTime)

				nearPly:falloutNotify("➲ You've received the blessing of Mercury", "shelter/sfx/collect_stimpack.ogg")
				nearPly:ScreenFade(SCREENFADE.IN, Color(0,255,0, 50), 0.3, 0)
				nearPly.SpeedBannerBuffActive = true

				local target = nearPly

				timer.Simple(0, function()
					ParticleEffectAttach("_sai_green_fire_3", PATTACH_POINT_FOLLOW, nearPly, 0)
				end)
				...
				timer.Simple(self.BuffExpireTime, function()
					if IsValid(target) and target.SpeedBannerBuffActive then
						target.SpeedBannerBuffActive = nil
					end
				end)
