if SERVER then
	concommand.Add("skybreaker_set_dest",function(ply) -- Set the primary teleport destination
		...
		local tppoints = file.Read("relay/skybreaker_dest.txt","DATA")
		...
		file.Write("relay/skybreaker_dest.txt",util.TableToJSON(tpTable))

		ply:ChatPrint("[NOTICE] You've set the relay point to your current position")
	end)

	concommand.Add("skybreaker_set_exit",function(ply)	-- Set the exit destination
		if !IsValid(ply) or !ply:IsSuperAdmin() then return end
		...
		local tppoints = file.Read("relay/skybreaker_exit.txt","DATA")
		if !tppoints or string.len(tppoints) <= 0 then tppoints = "" end
		local tpTable = util.JSONToTable(tppoints)
		if !tpTable then tpTable = {} end

		tpTable[game.GetMap()] = ply:GetPos()
		file.Write("relay/skybreaker_exit.txt",util.TableToJSON(tpTable))
...
SWEP.PrintName		= "Skybreaker Relay"
SWEP.Slot			= 0
SWEP.SlotPos		= 0
...
function SWEP:GetRelayPosition()
	local fl = file.Read("relay/skybreaker_dest.txt","DATA") or false
...
function SWEP:GetExitPosition()
	local fl = file.Read("relay/skybreaker_exit.txt","DATA") or false
