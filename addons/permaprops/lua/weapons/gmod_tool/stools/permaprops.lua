...
PermaProps.EntsBlacklist = {
	["nut_stash"] = true,
	["workbench"] = true,
	["nut_vendor"] = true,
	["harvestableplant"] = true,
	["faction-storage"] = true,
	["fo_mine_rock"] = true
}

hook.Add("ShouldPermaProp", "PermaPropBlacklist", function(ent, class)
	if class:StartWith("casinokit") or PermaProps.EntsBlacklist[class] then
		return false
	end
end)
...
function TOOL:LeftClick(trace)
	...
	local shouldPermaProp = hook.Run("ShouldPermaProp", ent, ent:GetClass())
	if shouldPermaProp == false then
		ply:ChatPrint("You cannot perma prop this entity.")
		return
	end
	...
	DiscordEmbed(ply:Nick() .. " ( " .. ply:SteamID() .. " ) has added a perma prop with a model of: " .. ent:GetModel() or "NA", "🏠 Perma Prop Update Log 🏠", Color(255,0,0), "BTeam")
...
function TOOL:RightClick(trace)
	...
	if ent:GetModel() == "models/error.mdl" then -- Custom "if then" added to remove Models that no longer exist on server, by Keith
		ply:ChatPrint("You erased an invalid model from the database")
		ent:Remove()
		return true
	end
...
	DiscordEmbed(ply:Nick() .. " ( " .. ply:SteamID() .. " ) has removed a perma prop with a model of: " .. ent:GetModel() or "NA", "🏠 Perma Prop Update Log 🏠", Color(255,0,0), "BTeam")
