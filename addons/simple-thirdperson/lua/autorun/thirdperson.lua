...
if CLIENT then
	...
	CreateClientConVar("simple_thirdperson_scoped", "0", true, false)
	...
	CreateClientConVar("simple_thirdperson_freelook", "1", true)
	...
	Editor.EnableScopeToggle = GetConVar( "simple_thirdperson_scoped" ):GetBool() or false
	...
	function BuildMenu(PNL)
		...
		Editor.PANEL.Freelook = Editor.PANEL.Sheet:Add( "DPanelSelect" )
		Editor.PANEL.Sheet:AddSheet( "Freelook", Editor.PANEL.Freelook, "icon16/arrow_refresh.png" )
		...
		Editor.PANEL.EnableFreelook = Editor.PANEL.Freelook:Add("DButton")
		Editor.PANEL.EnableFreelook:SetSize(250, 20)
		Editor.PANEL.EnableFreelook:SetPos(10, 6)

		local freelookConvar = GetConVar("simple_thirdperson_freelook")

		if freelookConvar:GetBool() then
			Editor.PANEL.EnableFreelook:SetText("Disable Freelook")
			Editor.PANEL.EnableFreelook:SetTextColor(Color(150,0,0))
		else
			Editor.PANEL.EnableFreelook:SetText("Enable Freelook")
			Editor.PANEL.EnableFreelook:SetTextColor(Color(0,150,0))
		end

		Editor.PANEL.EnableFreelook.DoClick = function()
			freelookConvar:SetBool(!freelookConvar:GetBool())
		...
		Editor.PANEL.EnableThrdScoped = Editor.PANEL.Freelook:Add( "DButton" )
		Editor.PANEL.EnableThrdScoped:SizeToContents()
		if Editor.EnableScopeToggle then
			Editor.PANEL.EnableThrdScoped:SetText("Disable Scope Toggle")
			Editor.PANEL.EnableThrdScoped:SetTextColor(Color(150,0,0))
		else
			Editor.PANEL.EnableThrdScoped:SetText("Disable Scope Toggle")
			Editor.PANEL.EnableThrdScoped:SetTextColor(Color(0,150,0))
		end
		Editor.PANEL.EnableThrdScoped:SetPos(10,35)
		Editor.PANEL.EnableThrdScoped:SetSize(250,20)
		Editor.PANEL.EnableThrdScoped.DoClick = function()
			Editor.EnableToggle = !Editor.EnableToggle
			RunConsoleCommand("simple_thirdperson_scoped",BoolToInt(Editor.EnableToggle))
			if Editor.EnableToggle then
				Editor.PANEL.EnableThrdScoped:SetText("Disable Scope Toggle")
				Editor.PANEL.EnableThrdScoped:SetTextColor(Color(150,0,0))
			else
				Editor.PANEL.EnableThrdScoped:SetText("Enable Scope Toggle")
				Editor.PANEL.EnableThrdScoped:SetTextColor(Color(0,150,0))
			end
		end
	...
	hook.Add("HUDShouldDraw", "SimpleTP.HUDShouldDraw", function(name)
		...
		if name == "CHudCrosshair" and GetConVar( "simple_thirdperson_enabled" ):GetBool() and (GetConVar( "simple_thirdperson_enable_custom_crosshair" ):GetBool() or GetConVar( "simple_thirdperson_hide_crosshair" ):GetBool()) then
	...
	local wasFreeLookEnabled = false
	local ranThisTick = false

	local function IsScoped(wep)
		local base = wep.Base

		if wep.IsCurrentlyScoped and wep:IsCurrentlyScoped() then
			return true
		end

		if base == "boh_scoped_base" and wep.Scoped and wep.Owner:KeyDown(IN_ATTACK2) then
			return true
		end
	...
	hook.Add("CalcView", "SimpleTP.Camera.View", function(ply, pos, angles, fov)
		...
		if GetConVar( "simple_thirdperson_scoped" ):GetBool() then
			if !vgui.CursorVisible() and ply:isWepRaised() and IsScoped(ply:GetActiveWeapon()) then
				wasEnabled = wasEnabled or isEnabled
				GetConVar("simple_thirdperson_enabled"):SetBool(false)
			elseif wasEnabled then
				GetConVar("simple_thirdperson_enabled"):SetBool(wasEnabled)
				wasEnabled = false
			end
		end
		...
		if isEnabled and IsValid(ply) then
			...
			if GetConVar("simple_thirdperson_freelook"):GetBool() and ply:Alive() and !ply:isWepRaised() and !ply:KeyDown(bit.bor(IN_FORWARD, IN_BACK, IN_MOVELEFT, IN_MOVERIGHT)) and ply:GetVelocity():Length() < 10 then
				ply.FreeAng = ply.FreeAng or angles
				angles = Angle(ply.FreeAng.p, ply.FreeAng.y, 0)
				...
				wasFreeLookEnabled = true
			else
				if wasFreeLookEnabled then
					wasFreeLookEnabled = false
					local ang = ply.FreeAng or ply:EyeAngles()
	...
	hook.Add("InputMouseApply", "SimpleTP.Camera.Free", function(cmd, x, y, ang)
		local ply = LocalPlayer()
		local isEnabled = GetConVar("simple_thirdperson_enabled"):GetBool() and GetConVar("simple_thirdperson_freelook"):GetBool()

		if ply.FreeAng and isEnabled and ranThisTick then
			ply.FreeAng.p = math.Clamp(math.NormalizeAngle(ply.FreeAng.p + y / 50), -85, 85)
			ply.FreeAng.y = math.NormalizeAngle(ply.FreeAng.y - x / 50)
