...
function SWEP:Shoot()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer()) ) then
		...
		dmginfo:SetDamage( 5 )
		dmginfo:SetDamageForce( self.Owner:GetForward() * 2000 )
		
		tr.Entity:TakeDamageInfo( dmginfo )
		
		if tr.Entity:IsPlayer() then
			if tr.Entity:GetNWBool("MAGuardening")==false then
		...
		if tr.Entity:IsNPC() and tr.Entity:GetClass() == "npc_metropolice" then
			if tr.Entity:GetActiveWeapon():GetClass() == "npc_cultblade" then
				if tr.Entity:GetNWBool("MAGuardening")==false then
					--tr.Entity:StopMoving()
					--tr.Entity:ExitScriptedSequence()
					tr.Entity:GetActiveWeapon():Stun2()
					tr.Entity:EmitSound("physics/metal/metal_grate_impact_hard3.wav")
				end
			end
		end
		if SERVER then
	...
	if ( SERVER && IsValid( tr.Entity ) ) then
		...
		if ( IsValid( phys ) ) then
			phys:ApplyForceOffset( self.Owner:GetAimVector() * 2 * phys:GetMass(), tr.HitPos )
		end
	end
...
SWEP.ViewModelBoneMods = {
	["TrueRoot"] = { scale = Vector(1, 1, 1), pos = Vector(-1.244, 2.798, 0.621), angle = Angle(0, 0, 0) },
	["RW_Weapon"] = { scale = Vector(0.009, 0.009, 0.009), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) },
	["LeftArm_1stP"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, -4.664), angle = Angle(0, 0, 0) }
}

SWEP.VElements = {
	["hammer"] = { type = "Model", model = "models/props_junk/garbage_glassbottle003a.mdl", bone = "RW_Weapon", rel = "", pos = Vector(0.518, 0.518, 8.831), angle = Angle(-178.831, 99.35, -8.183), size = Vector(0.56, 0.56, 0.56), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["hammer1"] = { type = "Model", model = "models/props_junk/garbage_glassbottle003a.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.635, 1.557, -4.676), angle = Angle(0, 0, 0), size = Vector(0.885, 0.885, 0.885), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
