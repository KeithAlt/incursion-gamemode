...
hook.Add("EntityTakeDamage", "ZZZZZ_Melee_ScaleDamageAsLegion", function(target, dmg)
	if !IsValid(target) or !target:IsPlayer() or !IsValid( target:GetActiveWeapon() ) or target:GetActiveWeapon().Base != "dangumeleebase" then return end 
	
	if nut.faction.indices[target:Team()] and nut.faction.indices[target:Team()].uniqueID == "legion" then
		dmg:ScaleDamage(1 - (20 / 100))
	end
end)
...
function SWEP:OnDeploy()
	...
	if nut and nut.faction.indices[ply:Team()] and nut.faction.indices[ply:Team()].uniqueID == "legion" and IsFirstTimePredicted() then
...
function SWEP:Think()
	...
	if ply:KeyDown(IN_ZOOM) then -- a scuffed exploit fix for really scuffed weapon code
	...
	elseif self.Owner:KeyReleased(IN_ATTACK) and self.Charge>0 then
		...
		if ply:Alive() and IsValid(ply) then
			...
			timer.Simple(0.1, function() -- Timer added to track dmginfo of a charged strike
				ply:SetNWBool("chargemaxxxed",false)
			end)
		end
...
elseif self.Owner:KeyDown(IN_BULLRUSH) then --Throwing -- [DISABLED]
...
function SWEP:AtkExtra()
	if (self.CanKnockout) then
		-- Put code here for knocking out.
	elseif (self.CanDecapitate) then
		-- Put code for decapitation
	end
end
...
function SWEP:Reload()
	...
	if CurTime() < self.NextFireShove + 2 then return end
	...
	if SERVER and self.Owner:isWepRaised() == true then
...
self:InitFunc()
	...
	if self.QuadAmmoCounter == true then
		...
		self.JumpPower=self.Owner:GetJumpPower()
...
function SWEP:HolsterExtra()
	...
	if self.Type==7 and self.Owner then
		if self.Weapon:GetNWInt("QSChain")>0 then
			self.Weapon:SetNWInt("QSChain",0)
			self.Owner:EmitSound("ambient/levels/canals/windchime2.wav",40,80)
		end
...
function SWEP:Holster()
	...
	if IsValid(ply) and IsValid(self) and nut and nut.faction.indices[ply:Team()] and nut.faction.indices[ply:Team()].uniqueID == "legion" then
		ply:SetRunSpeed( ply:GetRunSpeed() - 30 )
	end
