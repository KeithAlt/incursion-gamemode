...
function command:OnPlayerExecute(player, target)
	...
	if (position and pk_pills and !pk_pills.getMappedEnt(target)) then -- Edited by Keith, added pill compatibility
		target.sg_LastPosition = target:GetPos();
		target.sg_LastAngles = target:EyeAngles();
		target:SetPos(position);

		return true;
	elseif (position and pk_pills and pk_pills.getMappedEnt(target)) then
		target:SetPos(position)
		pk_pills.getMappedEnt(target):SetPos(position)
	return true;
	end;
end;
