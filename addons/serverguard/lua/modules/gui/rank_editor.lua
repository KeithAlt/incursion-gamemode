		...
		function panel:OnMousePressed()
			local menu = DermaMenu()
				...
				local option = menu:AddOption("Change image", function()
					...
					local leftPad, _, rightPad, _ = baseb:GetDockPadding()
					local boxSize = 18
					local grid = vgui.Create("DGrid")
					// grid:Dock(FILL)
					grid:SetCols(math.floor((baseb:GetWide() - leftPad - rightPad - 1) / boxSize) - 1)
					grid:SetColWide(boxSize)
					grid:SetRowHeight(boxSize)

					textureList:AddPanel(grid)

					local lastSelected

					for k, v in pairs(images) do
						local icon = grid:Add("DImageButton")
						// icon:Dock(LEFT)
						// icon:DockMargin(2, 0, 2, 0)
