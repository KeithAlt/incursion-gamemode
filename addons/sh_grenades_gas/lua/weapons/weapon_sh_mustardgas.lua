...
if (CLIENT) then
	SWEP.PrintName = "Mustard Gas Grenade"
	SWEP.Instructions = "Pull the pin and throw."
	SWEP.Purpose = "A grenade which disorientates and damages anyone unfortunate enough to walk into its gas cloud."
	SWEP.WepSelectIcon = surface.GetTextureID("weapons/weapon_sh_mustardgas")
...
game.AddAmmoType({name = "Mustard Gas Grenades"})

SWEP.Primary.ClipSize = 1
SWEP.Primary.DefaultClip = 2
SWEP.Primary.Ammo = "Mustard Gas Grenades"
...
SWEP.ThrownProjectile = "proj_sh_mustardgas"
SWEP.ThrowDelay = 0.2

-- SWEP defaults, overriden by mustardgas_settings.lua.
SWEP.AllowCooking = true
SWEP.CookTime = 3
SWEP.ThrowForce = 1250

function SWEP:SetupVars()
	self.AllowCooking = MUSTARDGAS_SETTINGS.AllowCooking
	self.CookTime = MUSTARDGAS_SETTINGS.CookTime
	self.ThrowForce = MUSTARDGAS_SETTINGS.ThrowForce
	self.detonate_timer = self.CookTime

	if (!MUSTARDGAS_SETTINGS.AllowAmmo) then
...
if (SERVER) then
	...
	function SWEP:PreDrawViewModel(vm)
		if (vm) then
			vm:SetSubMaterial(0, "models/weapons/v_models/eq_mustardgas/mustardgas")
		end
	end
