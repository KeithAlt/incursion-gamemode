...
pk_pills.register("deathclaw_enclave", {
    printName = "Deathclaw (Enclave)",
    side = "hl_antlion",
    type = "ply",
    model = "models/fallout/deathclaw_alphamale.mdl",
		...
		func=function(ply,ent)
			...
			timer.Simple(1, function()
				if IsValid(ent) and ply:Alive() then
					...
					for k,v in pairs(ents.FindInSphere(ent:GetPos() + ent:GetForward() * 240,125)) do
						if (v:IsPlayer() or v:IsNPC()) and (v != ply or v != ent) then
