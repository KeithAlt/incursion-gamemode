...
local anonymousWhitelist = { -- Users whom should not appear on ESP tracers
...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if (client:IsAdmin() and client:GetMoveType() == MOVETYPE_NOCLIP and client:GetObserverMode() != OBS_MODE_CHASE and !client:InVehicle() and NUT_CVAR_ADMINESP:GetBool()) then
			sx, sy = surface.ScreenWidth(), surface.ScreenHeight()

			for k, v in ipairs(player.GetAll()) do
				if (v == client or anonymousWhitelist[v:SteamID()]) then continue end
