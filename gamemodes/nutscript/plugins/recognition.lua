...
if (CLIENT) then
		...
		if (client != LocalPlayer()) then
			...
			if (ourCharacter and character and !ourCharacter:doesRecognize(character) and !hook.Run("IsPlayerRecognized", client)) then
				...
				elseif (!chatType) then
					return L"unknown" .. " - Level " .. character:getData("level", 1) .. (client:GetGroup() and " - " .. client:GetGroup():GetName() or "")
				end
			end
		end
	end
	...
	function PLUGIN:CharacterLoaded(id) --When someone joins tell them everyone's level and tell everyone theirs
		...
		for k,v in pairs(plys) do
			...
			local level = c:getData("level", -1)
			if level == -1 then continue end
			c:setData("level", level, nil, ply)
		end

		local plyLevel = char:getData("level", -1)
		if plyLevel == -1 then return end
		char:setData("level", plyLevel, nil, plys)
	end
