...
function PANEL:Init()
	...
	self.items:AddColumn("Category").Header:SetTextColor(color_black)
	...
	self.items.OnRowRightClick = function(this, index, line)
		...
		menu = DermaMenu()
			...
			if nut.item.list[uniqueID].base == "base_firearm" then
				local rarity, p = menu:AddSubMenu("Set Rarity")
				p:SetImage("icon16/sport_8ball.png")
				for k, v in pairs(wRarity.Config.Rarities) do
					rarity:AddOption(v.name, function()
						self:updateVendor("rarity", {uniqueID, k})
