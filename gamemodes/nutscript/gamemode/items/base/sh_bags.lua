	...
	onClick = function(item)
		...
		if (index) then
			...
			if (inventory and inventory.slots) then
				local userInventory = client:getChar():getInv()
				if!(userInventory and userInventory:getItemByID(item.id)) then
					nut.util.notify("You cannot do this.")
					return false
				end
				...
				if IsValid(nut.gui.inv1) then
					panel:MoveRightOf(nut.gui.inv1, 1)
				end
