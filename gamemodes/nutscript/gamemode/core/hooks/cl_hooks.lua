...
function GM:DrawEntityInfo(entity, alpha, position)
	if (entity.IsPlayer(entity)) then
		...
		if (character) then
			...
			charInfo = {}
			charInfo[1] = {hookRun("GetDisplayedName", entity) or (character.getName(character) .. " - Level " .. character:getData("level", 1) .. (entity:GetGroup() and " - " .. entity:GetGroup():GetName() or "") .. (FalloutGuilds.GetGuild(entity) and " - " .. FalloutGuilds.GetGuild(entity).name or "")), jlib.GetTeamColor(entity)}
