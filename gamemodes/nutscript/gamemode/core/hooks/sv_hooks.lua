...
function GM:PlayerLoadedChar(client, character, lastChar)
	...
	if (character) then -- Checks on initial load of char if has class data and to assign accordingly
		local class = character:getData("class") or nut.class.list[character:getClass()]

		for k, v in ipairs(nut.class.list) do
			if v.uniqueID == class then
				character:setClass(k)
				character:setData("class", v.uniqueID)
				break
			end
	...
	if (faction and faction.pay and faction.pay > 0) then
		timer.Create("nutSalary" .. client:UniqueID(), faction.payTime or 300, 0, function()
			-- NOTE: Addresses a bug where the character continues to receive pay after switching
			if IsValid(client) and !(character or client:getChar()) or character != client:getChar() then
				timer.Remove("nutSalary" .. client:UniqueID())
				ErrorNoHalt(client:Nick() .. " no longer has a character loaded; removing faction pay timer")
				return
			end

			local pay = hook.Run("GetSalaryAmount", client, faction) or faction.pay
			pay = hook.Run("GetSalaryAmountExtra", pay, client, faction) or pay
...
function GM:PlayerLoadout(client)
	...
	if (character) then
		...
		if (faction) then
			...
			if (faction.weapons and (class and !class.weapons)) then
...
function GM:PlayerDisconnected(client)
	...
	if timer.Exists("nutSalary" .. client:UniqueID()) then -- Fixes a logic mistake with faction pay
		timer.Remove("nutSalary" .. client:UniqueID())
	end
end
