...
if (SERVER) then
	...
	function META:add(uniqueID, quantity, data, x, y, noReplication)
		...
		if isnumber(uniqueID) then
			...
			isBag = item and item.base == "base_bags"
		elseif isstring(uniqueID) then
			local item = nut.item.list[uniqueID]
			if item then
				isBag = item.base == "base_bags"
			else
				isBag = false
			end
		end
		...
		if (quantity > 0) then
			...
			if (type(uniqueID) == "number") then
				...
				if (item) then
					if (!x and !y) then
						x, y, bagInv = self:findEmptySlot(item.width, item.height, isBag)
					end
				...
				if (!x and !y) then
					x, y, bagInv = self:findEmptySlot(itemTable.width, itemTable.height, isBag)
				end
