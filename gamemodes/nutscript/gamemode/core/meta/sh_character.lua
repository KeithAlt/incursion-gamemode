...
if (SERVER) then
	...
	-- Save only a particular character variable
	function CHAR:saveSingle(callback, var, force)
		if self.isBot then return end

		local field = nut.char.vars[var].field
		local data = {[field] = self.vars[var]}

		local shouldSave = hook.Run("CharacterPreSave", self)

		if force or shouldSave != false then
			nut.db.updateTable(data, function()
				if callback then
					callback()
				end
	...
	hook.Add("OnCharVarChanged", "SaveMoney", function(char, varName)
		if varName == "money" then
			char:saveSingle(nil, varName, true)
		end
	end)
	...
	function CHAR:kick()
		...
		-- Reset all armor models
		client:ArmorReset()
		client:ResetAccessories()
		-- Set model back to default, same as what it would be the first time
		-- they connect before they pick a character
		client:SetModel("models/player.mdl")
		-- Spawn the player as spectator so they are not standing anywhere.
		...
		if (self and self.steamID == steamID) then
			...
			hook.Run("OnCharKicked", client, self)
		end
	end
