...
function ITEM:setData(key, value, receivers, noSave, noCheckEntity)
	local previousValue = self:getData(key, "no value")
	...
	if key == "rarity" then
		print("setData called for " .. self:getName() .. " item ID: " .. self:getID() .. " value: " .. tostring(value) .. " previous value: " .. tostring(previousValue))
		debug.Trace()
	end
end
