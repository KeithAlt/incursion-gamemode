...
if (SERVER) then
	function ENT:Initialize()
		...
		self.dummy:SetModel("models/fallout/dungeons/enclave/ecvdoorbg01.mdl")
		self.dummy:DrawShadow(false)
		self.dummy:SetPos(self:GetPos())
		self.dummy:SetAngles(self:GetAngles() + Angle(0, 90, 0))
	...
	function ENT:Use(player)
		...
		if (self:getNetVar("password") == false) then
			if (self:getNetVar("open") != true) then
				...
				self:EmitSound("fallout/doors/drs_enclave_large_open.wav", 65)
				timer.Simple(1.5, function() self:SetCollisionGroup(COLLISION_GROUP_DEBRIS) end)
			else
				self.dummy:Fire("SetAnimation", "close", 0)
				self:setNetVar("open", false)
				self:EmitSound("fallout/doors/drs_enclave_large_close.wav", 65)
