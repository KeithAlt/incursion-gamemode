ITEM.name = "[Base Utility] Stationary Radio"
...
function ITEM:getDesc()
	...
	if (!self.entity or !IsValid(self.entity)) then
		str = "A Radio that allows you to contact others that have a personal radio on a specified frequency between both you and them.\nPower: %s"
		return Format(str, (self:getData("power") and "On" or "Off"), self:getData("freq", "000.0"))
	else
		local data = self.entity:getData()

		str = "A Functional Pager. Power: %s"
