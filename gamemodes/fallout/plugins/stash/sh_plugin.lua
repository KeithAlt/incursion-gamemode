				...
				if (data and #data > 0) then
					for k, v in ipairs(data) do
						...
						for itemID,_ in pairs(data) do
							nut.db.query("SELECT * FROM nut_items WHERE _itemID = " .. itemID .. ";", function(dat)
								if !dat then
									print("Removing item with ID " .. itemID .. " from " .. char:getName() .. "'s stash since it doesn't exist in the DB.")
									data[itemID] = nil
									return
								end

								if nut.item.list[dat[1]._uniqueID] == nil then
									print("Removing item with ID " .. itemID .. " from " .. char:getName() .. "'s stash since it has an invalid uniqueID: '" .. dat[1]._uniqueID .. "'.")
									data[itemID] = nil
									return
								end
							end)
