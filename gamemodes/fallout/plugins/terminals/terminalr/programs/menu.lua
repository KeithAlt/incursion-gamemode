...
TerminalR.Programs["menu"].Init = function( self )
	...
	local SW, SH = TerminalRGetTextSize("> CREATE NEW DISK", "TrmR_Medium")
	local CreateNewDisk = self:CreateMenuButton("CreateNewDisk", "> CREATE NEW DISK")
	CreateNewDisk:SetPos( W*0.05, H*0.15 )
	CreateNewDisk:SetSize( SW + 40 , SH + 10 )
	CreateNewDisk.DoClick = function( selfB )
		--program for writing a new disk, only has print options
		if ( self.Memory.DiskText ) then
			self:SetMemory( "MenuMsg", "Eject current disk to create a new one.")
			self:SetMemory( "MenuMsgAlpha", 255 )
			surface.PlaySound( "terminalr/false.wav" )
		else	
			self:LaunchProgram( "disk_write_n" )
		end
	end		
	...
	WriteOnDisk:SetPos( W*0.05, H*0.25 )
	WriteOnDisk:SetSize( SW + 40 , SH + 10 )
	WriteOnDisk.DoClick = function( selfB )
		--dont let people write on ecrypted disks
		if(self:GetMemory("Encrypt")) then 
			self:SetMemory( "MenuMsg", "Disk is encrypted.")
			self:SetMemory( "MenuMsgAlpha", 255 )
			surface.PlaySound( "terminalr/false.wav" )
	...
	local SW, SH = TerminalRGetTextSize("> PASSWORD DISK", "TrmR_Medium" )
	local ProtectDisk = self:CreateMenuButton("ProtectDisk", "> PASSWORD DISK")
	ProtectDisk:SetPos( W*0.05, H*0.35 )
	...
	local SW, SH = TerminalRGetTextSize("> ENCRYPT DISK", "TrmR_Medium" )
	local EncryptDisk = self:CreateMenuButton("EncryptDisk", "> ENCRYPT DISK")
	EncryptDisk:SetPos( W*0.05, H*0.4 )
	EncryptDisk:SetSize( SW + 40 , SH + 10 )
	EncryptDisk.DoClick = function( selfB )
		self:SetMemory("MenuMsg", "DISK ENCRYPTED")
		self:SetMemory("MenuMsgAlpha", 255)
	
		self:SetMemory("Encrypt", true)
		--[[
		if ( self:GetMemory("DiskText") ) then
			if ( self:GetMemory("DiskPass") ) then
				self:SetMemory("BlockedProgram", "password_save" )
	...
	local SW, SH = TerminalRGetTextSize( "> " .."SAVED HOLOLOGS", "TrmR_Medium" )
	local FavoriteMenu = self:CreateMenuButton( "SavedHolologs", "> SAVED HOLOLOGS")
	FavoriteMenu:SetPos( W*0.05, H*0.45 )
	FavoriteMenu:SetSize( SW + 40 , SH + 10 )
	FavoriteMenu.DoClick = function( selfB )
		self:LaunchProgram("disk_favorite")
