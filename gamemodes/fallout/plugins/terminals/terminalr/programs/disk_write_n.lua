TerminalR.Programs["disk_write_n"] = {}

TerminalR.Programs["disk_write_n"].Init = function( self )
	...
	Save.DoClick = function(selfB)
		local author = self:GetMemory("DiskAuthor")
		if(!author) then
			self:SetMemory("DiskAuthor", LocalPlayer():Name())
		else
			self:SetMemory("DiskEditor", LocalPlayer():Name())
		end	
	...
	local Print = self:CreateMenuButton("Print", "> PRINT")
	Print:SetPos( ScrW()*0.24, ScrH()*0.925 )
	Print:SetSize( SW + 40, SH + 10 )
	Print.DoClick = function( selfB )
		local info = {}
		--info.text = self:GetMemory("DiskText")
		info.author = LocalPlayer():Name()
		info.editor = LocalPlayer():Name()
	...
	local SW, SH = TerminalRGetTextSize("> RENAME", "TrmR_Medium")
	local Save = self:CreateMenuButton("Rename", "> RENAME")
	Save:SetPos(ScrW()*0.8, ScrH()*0.925)
	Save:SetSize(SW + 40, SH + 10)
	Save.DoClick = function(selfB)
		Derma_StringRequest("Rename Disk", "", "Disk", function(text)
			if(text) then
				self:SetMemory("DiskTitle", text)
			end
...
TerminalR.Programs["disk_write_n"].OnRemove = function( self )
	self:RemoveAllMemoryNamed( { "CharPosX", "CharLine", "LineDistance", "CamY" } )
end

TerminalR.Programs["disk_write_n"].Paint = function( self, w, h )
	local Text = self:GetMemory("DiskText")[self:GetMemory("CharLine")] or ""
	local CharX = self:GetMemory( "CharPosX" )
