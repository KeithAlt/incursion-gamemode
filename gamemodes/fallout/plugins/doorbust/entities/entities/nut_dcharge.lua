...
if (SERVER) then
	...
	function ENT:Explode()
		...
		for k, v in ipairs(doors) do
			if v:GetClass() == "nut_anidoor_utilsmall" or v:GetClass() == "nut_anidoor_utillarge" then
				v:Remove()
			end
			if (v:isDoor()) then
				local dir = self:GetPos()
				v:blastDoor(dir, 1, true)
			end
		...
		util.Effect("effect_fo3_smoke", effect)

		for index, entity in pairs(ents.FindInSphere(self:GetPos(), 900)) do
			if entity:IsPlayer() and entity:Alive() then
				entity:ScreenFade(SCREENFADE.IN, Color(255,225,225,75), 0.5, 0)
			end
		end
