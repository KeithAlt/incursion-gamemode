		...
		for i, v in SortedPairs(bottles) do
			if (stock[i]) then
				self.fridge[i] = ents.Create("prop_dynamic")
					self.fridge[i]:SetModel(nut.item.list[stock[i]].model or "models/mosi/fallout4/props/drink/sunsetsarsaparilla.mdl")
			...
			if (item) then
				...
				timer.Simple((self.cost / 2) + 1, function()
					...
					timer.Simple(1, function()
						...
						timer.Simple(1, function() self:EmitSound("fallout/nukamachine/sunset.wav") end)
					end)
				end)
			else
				self.nextUse = CurTime() + 1
