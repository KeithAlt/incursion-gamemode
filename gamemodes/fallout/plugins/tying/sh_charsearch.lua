...
local noSteal = {
	...
	["implant_chemist"] = true,
	["implant_tamer"] = true,
	["implant_robotics"] = true,
	["implant_nuclear"] = true,
	["implant_jurryrigging"] = true,
	["implant_interrogator"] = true,
	["implant_intel"] = true,
	["implant_handy"] = true,
	["implant_hacker"] = true,
	["implant_gunnut"] = true,
	...
	["implant_doctor"] = true,
	["implant_demo"] = true,
	["implant_combatant"] = true,
	["implant_chemist"] = true,
	["implant_black"] = true,
	["radio"] = true,
	["bos_civilian_tag"] = true,
	["legion_loyalist_coin"] = true,
	["american_citizenship_id_card"] = true,
	["pla_civilian_papers"] = true,
	["regiis_devotee_charm"] = true,
	["ncr_id_card"] = true,
	["ncrid"] = true,
	["children_of_atom_charm"] = true,
	["blessing_of_kiyya"] = true,
	["children_of_the_cathedral_badge"] = true,
	["followers_of_the_apocalypse_principle"] = true,
	["cult_of_mars"] = true,
	["mormon_church_bible"] = true,
	["big"] = true,
	["medic"] = true,
	["small"] = true,
	["ringcase"] = true
...
function PLUGIN:CanItemBeTransfered(item, oldInv, newInv)
	if(newInv and oldInv and newInv != oldInv) then
		local storage = (newInv.vars and newInv.vars.isStorage) or (oldInv.vars and oldInv.vars.isStorage)

		if (!storage and oldInv.owner and newInv.owner and noSteal[item.uniqueID]) then
			return false
		end
	end
end
...
if (SERVER) then
	function PLUGIN:searchPlayer(client, target)
		...
		if SERVER then -- Custom func added by Keith to relay via Discord logging
			DiscordEmbed(client:Nick() .. " ( " .. client:SteamID() .. " ) " .. "has searched the inventory of " .. target:Nick() .. "( " .. target:SteamID() .. " )", "Inventory Search Log", Color(255,165,0), "Admin")
		end
...
nut.command.add("charsearch", {
	onRun = function(client, arguments)
		...
		if (IsValid(target) and target:IsPlayer() and target:Alive() and target:getNetVar("restricted")) then
