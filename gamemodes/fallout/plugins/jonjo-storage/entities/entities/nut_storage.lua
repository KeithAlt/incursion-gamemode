...
if (SERVER) then
	...
	function ENT:OpenInv(activator)
		if IsValid(self.User) and self.User != activator and self.SingleUser then
			activator:notify("Someone else is using this!")
			return
		end
	...
	function ENT:Use(activator)
		...
		if (inventory and (activator.nutNextOpen or 0) < CurTime()) then
			if (activator:getChar()) then
				...
				if (self:getNetVar("faction")) then
					self:EmitSound(def.locksound or "doors/default_locked.wav")
					if (activator:Team() != self:getNetVar("faction")) then
						return
					end;
				end;
