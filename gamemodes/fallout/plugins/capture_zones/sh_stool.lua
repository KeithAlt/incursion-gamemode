/* Pasted from sandbox because Garry is an asshole and nils out the toolobj global var. */
ToolObj = {}
SWEP = weapons.GetStored("gmod_tool")

nut.util.include("sh_ghostentity.lua")
nut.util.include("sh_object.lua")
nut.util.include("cl_stool.lua")
...
local toolmodes = file.Find( PLUGIN.folder .. "/stools/*.lua", "LUA" )
...
for key, val in pairs( toolmodes ) do
	...
	AddCSLuaFile( PLUGIN.folder.."/stools/" .. val )
	include( PLUGIN.folder.."/stools/" .. val )
...
if ( CLIENT ) then
	...
	hook.Add( "PopulateToolMenu", "AddSToolsToMenu_nutscript", function()
