...
PANEL = {}
					...
					if (item and !IsValid(self.panels[item.id])) then
						...
						if (IsValid(icon)) then
							...
							if (newTooltip) then
								...
								icon:SetToolTip(Format(nut.config.itemFormat, "<color="..color.r..","..color.g..","..color.b..">"..item:getName().."</color>", item:getDesc() or ""))
							end
							icon.itemID = item.id
	...
	function PANEL:addIcon(model, x, y, w, h, skin)
		...
		if (self.slots[x] and self.slots[x][y]) then
			...
			if itemTable and (self.panels[itemTable:getID()]) then
				self.panels[itemTable:getID()]:Remove()
			end

			if (itemTable and itemTable.exRender) then
